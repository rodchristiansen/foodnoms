...
`foodnoms log` reported `log: submitted; the intent did not return. Verify with
foodnoms day` and nothing was in the log. The message reads like the known
...
library afterwards and says so. `FOODNOMS_BRIDGE` overrides the name the CLI dispatches to,
