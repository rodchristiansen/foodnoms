...
means the Shortcut hangs forever. `foodnoms` refuses up front and names what is missing. Pass
...
foodnoms log "Mediterranean Salad" --date "2026-09-03 12:30" \
...
real food, see [Correcting the Log](Correcting-The-Log.md) and `foodnoms-relog`.

## `create-food` — add a food to the library

```bash
foodnoms create-food "House Granola" --brand "Homemade" \
  --calories 220 --protein 6 --carbs 28 --fat 9
foodnoms create-food "Cold Brew" --barcode 0123456789012 --serving-size "250 ml"
```
