...
`foodnoms` walks the containers and matches each one's identity plist against FoodNoms'
bundle id.
...
`foodnoms doctor` is the tripwire: it fails loudly when the shape moves rather than returning
