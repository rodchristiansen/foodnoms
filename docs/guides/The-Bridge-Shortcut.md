...
import either is refused or arrives as `FoodNoms Bridge 2`. The `shortcuts` CLI is
...
```bash
export FOODNOMS_BRIDGE="FoodNoms Bridge 3"
```
...
```
  [WARN] write path: "FoodNoms Bridge" not installed — reads work, writes will fail.
  [note] "FoodNoms Bridge 3" is in the library but is not the name the CLI runs.
```
...
wrapping `GetFoodEntriesIntent` would be strictly worse than `foodnoms day` — so what is left
