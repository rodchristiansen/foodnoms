...
```bash
foodnoms day --json | jq '.totals.protein'
```
...
for i in $(seq 0 6); do
  d=$(date -v-${i}d +%Y-%m-%d)
  printf "%s  %s\n" "$d" "$(foodnoms day "$d" --json | jq '.totals.protein')"
done
```
...
```bash
foodnoms resolve "oat milk" --json | jq -r '.[0] | "\(.food_id)  \(.barcode)"'
```
...
```bash
foodnoms catalog --min-count 20 --json \
...
```bash
foodnoms search "skyr" --json | jq -r '.[] | "\(.count)  \(.name)  \(.last_logged)"'
```

## Export the barcode table for a device

```bash
foodnoms barcodes --json > known-foods.json
```
...
for i in $(seq 0 30); do
  d=$(date -v-${i}d +%Y-%m-%d)
  n=$(foodnoms day "$d" --json | jq '.entries | length')
