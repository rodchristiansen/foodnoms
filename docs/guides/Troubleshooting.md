...
Run `foodnoms doctor` first. It is the answer to most of what follows.
...
The container is TCC-protected. The process running `foodnoms` needs **Full Disk Access** —
...
```bash
foodnoms flush
tail ~/Library/Logs/foodnoms-flush.log
ls ~/.local/state/foodnoms/queue/pending/
```
