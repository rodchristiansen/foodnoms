...
# The CLI ships as `foodnoms` with no extension, so it is loaded by path.
_spec = importlib.util.spec_from_loader(
    "fnctl", SourceFileLoader("fnctl", os.path.join(HERE, os.pardir, "foodnoms")))
...
class StoreTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.tmp = tempfile.mkdtemp(prefix="foodnoms-tests.")
...
class TestBridgeInstalled(unittest.TestCase):
    ...
    # The bridge name is configurable (FOODNOMS_BRIDGE), so these anchor on
...
class TestBridgeVersioning(unittest.TestCase):
    ...
    def pick(self, names, env=None):
        real_names, real_env = fn.library_names, os.environ.get("FOODNOMS_BRIDGE")
        fn.library_names = lambda: names
        if env: os.environ["FOODNOMS_BRIDGE"] = env
        else: os.environ.pop("FOODNOMS_BRIDGE", None)
        try:
            return fn.find_bridge()
        finally:
            fn.library_names = real_names
            if real_env is not None: os.environ["FOODNOMS_BRIDGE"] = real_env
            else: os.environ.pop("FOODNOMS_BRIDGE", None)

    def test_newest_stamp_wins(self):
        self.assertEqual(self.pick([
            "Log Water",
            "FoodNoms Bridge 2026.09.07.0900",
            "FoodNoms Bridge 2026.09.07.1729",
            "FoodNoms Bridge 2026.08.30.2201",
        ]), "FoodNoms Bridge 2026.09.07.1729")

    def test_stamps_sort_by_time_not_string_length(self):
        self.assertEqual(self.pick([
            "FoodNoms Bridge 2026.09.07.0900",
            "FoodNoms Bridge 2026.10.01.0001",
        ]), "FoodNoms Bridge 2026.10.01.0001")
    ...
    def test_falls_back_to_a_legacy_name(self):
        self.assertEqual(self.pick(["FoodNoms Bridge 3"]),
                         "FoodNoms Bridge 3")

    def test_the_env_override_wins_over_everything(self):
        self.assertEqual(self.pick(["FoodNoms Bridge 2026.09.07.1729"],
                                   env="Some Other Bridge"), "Some Other Bridge")


_bspec = importlib.util.spec_from_loader(
    "fnbridge", SourceFileLoader("fnbridge", os.path.join(HERE, os.pardir, "foodnoms-bridge")))
